...
(*[TODO render the Ast during logging]*)
...
let rec process_file ~dev filename =
  if Filename.extension filename = ".tree" then
    ...
    let source_path =
      if dev then Some (Eio_posix.Low_level.realpath filename) else None
    in
    let code = parse_file filename in
    S.yield { source_path; addr; code }
  else if Sys.is_directory filename then process_dir ~dev filename

and process_dir ~dev dir =
  Sys.readdir dir
  |> Array.iter @@ fun filename -> process_file ~dev @@ dir ^ "/" ^ filename

let read_trees_in_dirs ~dev dirs =
  S.run @@ fun () -> dirs |> List.iter (process_dir ~dev)
*)
module Make (Message : Asai.MinimumSigs.Message) = struct
  ...
  let () =
    ...
    | LspError (HandshakeError err) ->
        Some
          (Format.asprintf "Lsp Error: Invalid initialization handshake %s" err)
  ...
  let render_lsp_diagnostic (uri : L.DocumentUri.t) (diag : diagnostic) :
    ...
    let severity =
      LspShims.Diagnostic.lsp_severity_of_severity @@ diag.severity
    in
    ...
    in
    Lsp_Diagnostic.create ~range ~severity ~code ?source ~message
      ~relatedInformation ()
  ...
  let load_file uri =
    ...
    let push_diagnostic d = diagnostics := d :: !diagnostics in

    (*[TODO Kento O. 12/13/2023 ]*)
    (*parse_file "hello.tree";*)
    (*let read_trees_in_dir ~dev dir = read_trees_in_dirs ~dev [ dir ] in
      read_trees_in_dir ~dev:true "change_me_to_the_lsp_root_path" ();
    *)
  ...
  let code_action (_params : L.CodeActionParams.t) : L.CodeActionResult.t = None

  (* [TODO: Reed M, 12/12/2022] No hovers for now. *)
  let hover (_params : L.HoverParams.t) : L.Hover.t option = None
  ...
  module Request = struct
    ...
    let dispatch : type resp. string -> resp t -> resp =
     fun mthd -> function
      | Initialize _ ->
          let err =
            "Server can only recieve a single initialization request."
          in
          raise @@ LspError (HandshakeError err)
      | Shutdown -> initiate_shutdown ()
      | CodeAction params -> code_action params
      | TextDocumentHover params -> hover params
    ...
    let dispatch : string -> t -> unit =
     fun mthd -> function
      | TextDocumentDidOpen doc ->
          Eio.traceln "Opened %s@" doc.textDocument.text;
  ...
  let run env ?source ~init ~load_file k =
    ...
    let init = { lsp_io; source; init; load_file; should_shutdown = false } in
    State.run ~init k
end
